...
router.get('/resumo', authenticateToken, async (req, res) => {
  try {
    ...
    const CAT = 'Reserva de Segurança';

    const [kpisRes, movRes, mediaSaidasRes] = await Promise.all([
      db.query(`
        SELECT
          COALESCE(SUM(CASE WHEN t.valor > 0 THEN t.valor ELSE 0 END), 0) AS aportes,
          COALESCE(SUM(CASE WHEN t.valor < 0 THEN ABS(t.valor) ELSE 0 END), 0) AS retiradas,
          COALESCE(SUM(t.valor), 0) AS saldo
        FROM transacoes t
        LEFT JOIN categorias c ON t.categoria_id = c.id
        WHERE t.usuario_id = $1 AND t.tipo = 'investimento' AND c.nome = $2
      `, [userId, CAT]),
